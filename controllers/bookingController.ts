...
export const getAvailableSlots = (ctx: any) => {
  ...
  const bookedSlots =
    AppointmentModel.getBookedSlotsForSupervisor(supervisorName);
...
export const bookAppointment = async (ctx: any) => {
  ...
  const isBooked = AppointmentModel.checkExistingBooking(supervisor, slot);
  if (isBooked) {
    ctx.response.status = 409;
    ctx.response.body = { error: "Slot already booked." };
    return;
  }
  AppointmentModel.createAppointment(student_id, supervisor, slot);
  ctx.response.body = {
    success: true,
